exports.result = async function prompt_MainMenu_result({ session, logger, action, user_input }) {
    const routine = `app:prompt_MainMenu_result`;
    ...
    if (intent_config && intent_config.data[action]) {
        ...
        } else if (auth_treatment === 'infinite_say') {
            // ambiguous action ==> need more specific intent
            next_action = 'infinite_loop';
        }
    } else {
